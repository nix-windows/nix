...
for /f %%i in ('%OLDNIX%\bin\nix-build.exe --keep-failed --option system x86_64-windows -o x86_64-boost            -E "with (import <nixpkgs> { }); pkgsMsvc2019.boost174.override{ staticRuntime=true; static=true; }"') do set BOOST=%%i
...
md                                       ..\builddir64-vs2019-mt

    %MESON%\mingw64\bin\meson setup      ..\builddir64-vs2019-mt . --backend vs2019 --default-library static --buildtype release ^
...
rem %MESON%\mingw64\bin\meson compile -C ..\builddir64-vs2019-mt --clean
    %MESON%\mingw64\bin\meson compile -C ..\builddir64-vs2019-mt --verbose
    %MESON%\mingw64\bin\meson install -C ..\builddir64-vs2019-mt
