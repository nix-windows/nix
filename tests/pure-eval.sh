...
if [[ "$(uname)" =~ ^MINGW|^MSYS ]]; then
    [[ $(nix eval --impure --expr "(import (builtins.fetchurl { url = file://$(cygpath -m $(pwd))/pure-eval.nix; })).x") == 123 ]]
    (! nix eval --expr "(import (builtins.fetchurl { url = file://$(cygpath -m $(pwd))/pure-eval.nix; })).x")
    nix eval --expr "(import (builtins.fetchurl { url = file://$(cygpath -m $(pwd))/pure-eval.nix; sha256 = \"$(nix hash-file pure-eval.nix --type sha256)\"; })).x"
else
