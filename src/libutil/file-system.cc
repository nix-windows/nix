...
void writeFile(const std::filesystem::path & path, std::string_view s, mode_t mode, FsSync sync)
{
    ...
    writeFile(fd, path.string(), s, mode, sync);

    /* Close explicitly to propagate the exceptions. */
    fd.close();
}

void writeFile(AutoCloseFD & fd, const std::filesystem::path & origPath, std::string_view s, mode_t mode, FsSync sync)
{
    assert(fd);
    try {
        writeFull(fd.get(), s);
        ...
        e.addTrace({}, "writing file %s", PathFmt(origPath));
        throw;
    }
}
...
std::filesystem::path makeTempPath(const std::filesystem::path & root, const std::string & suffix)
{
    ...
    return tmpRoot / fmt("%2%-%3%-%4%", suffix, getpid(), counter.fetch_add(1, std::memory_order_relaxed));
}
...
void moveFile(const std::filesystem::path & oldName, const std::filesystem::path & newName)
{
    try {
        ...
        std::filesystem::path temp =
            createTempDir(os_string_to_string(PathView{newPath.parent_path()}), "rename-tmp");
