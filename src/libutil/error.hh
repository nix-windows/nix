...
public:
    ...
    WinError(const Args & ... args)
        : SysError("")
    {
        err.hint = addLastError(hintfmt(args...));
    }

private:

	/* Sets `lastError` and provides a string explanation if possible. */
	hintformat addLastError(hintformat && hf);
};
#endif
