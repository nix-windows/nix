    ...
    std::function<ValidPathInfo(HashResult)> mkInfo)
{
    ...
    if (writeDebugInfo) {
        ...
        if (narAccessor->stat1(buildIdDir).type == FSAccessor::tDirectory) {
            ...
            for (auto & s1 : narAccessor->readDirectory(buildIdDir)) {
                ...
                if (narAccessor->stat1(dir).type != FSAccessor::tDirectory
                ...
                for (auto & s2 : narAccessor->readDirectory(dir)) {
                    ...
                    if (narAccessor->stat1(debugPath).type != FSAccessor::tRegular
