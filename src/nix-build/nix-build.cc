    ...
    MyArgs myArgs(myName, [&](Strings::iterator & arg, const Strings::iterator & end) {
        if (*arg == "--help") {
            ...
            if (std::regex_search(interpreter, std::regex("ruby"))) {
                ...
                envCommand = fmt("exec %1% %2% -e 'load(ARGV.shift)' -- %3% %4%", execArgs, interpreter, shellEscape(script.string()), joined.str());
            } else {
                envCommand = fmt("exec %1% %2% %3% %4%", execArgs, interpreter, shellEscape(script.string()), joined.str());
            }
        }
    ...
    if (readStdin)
        ...
        for (auto i : remainingArgs) {
            auto baseDir = inShebang && !packages
                ? fs::absolute(script.parent_path())
                : (fs::path{i});

            if (fromArgs)
                exprs.push_back(state->parseExprFromString(
                    std::move(i),
                    (inShebang && compatibilitySettings.nixShellShebangArgumentsRelativeToScript) ? lookupFileArg(*state, baseDir.string()) : state->rootPath(".")
                ));
            else {
                fs::path absolute = i;
                try {
